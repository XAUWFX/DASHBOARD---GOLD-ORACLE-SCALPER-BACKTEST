...
def month_metrics(df):
    ...
    if cur.empty:
        return dict(total=0, wins=0, losses=0, cumTV=0, profitFactor=0, winrate=0)
    wins   = cur[cur["win"] == True]
    losses = cur[cur["win"] == False]
    grossP = wins["pctTV"].sum()        if len(wins)   > 0 else 0
    grossL = abs(losses["pctTV"].sum()) if len(losses) > 0 else 0
    return dict(
...
.pnl-row{{display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.03);gap:8px;}}
...
<main class="container">
  ...
  <section class="section">
    ...
    <div class="section-sub">Cumulative TV % month by month — current year</div>
...
const kpiData=[
  ...
  {{label:"Month TV %",   val:fmtPct(mm.cumTV||0),  cls:(mm.cumTV||0)>=0?"green":"red", note:"Cumulative TV % this month"}},
];
...
if(activeWeeks.length===0){{
  ...
  D.weekly.forEach(w=>{{
    ...
    wEl.innerHTML+=`
      <div class="pnl-row">
        <span class="pnl-label">${{w.label}}<span class="pnl-meta">${{w.trades}}T · ${{w.wins}}W</span></span>
        <span class="pnl-val" style="color:${{clr(w.pnlTV)}}">${{fmtPct(w.pnlTV)}}</span>
      </div>`;
  }});
}}
...
if(!D.monthly||D.monthly.length===0){{
  ...
  [...D.monthly].reverse().forEach(m=>{{
    mEl.innerHTML+=`
      <div class="pnl-row">
        <span class="pnl-label">${{m.label}}<span class="pnl-meta">${{m.trades}}T · ${{m.wins}}W</span></span>
        <span class="pnl-val" style="color:${{clr(m.pnlTV)}}">${{fmtPct(m.pnlTV)}}</span>
      </div>`;
  }});
}}
  ...
  options:{{
    ...
    plugins:{{
      ...
      tooltip:{{
        ...
        callbacks:{{label:c=>" TV %: "+(c.parsed.y>=0?"+":"")+c.parsed.y.toFixed(2)+"%"}}
      }}
    }},
    scales:{{
